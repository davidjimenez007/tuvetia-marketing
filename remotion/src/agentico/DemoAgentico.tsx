/**
 * DemoAgentico · la composición (prompts/demo-vetgpt-agentico.md sobre la plantilla): reel 1080×1920 ·
 * 30 fps · 1500 frames, sólo escritorio. Capas (plantilla §3): nieve con grano → ventana de escritorio
 * (plano general con cromo al abrir; encuadres del asistente, la bandeja y la agenda después) → banda de
 * texto + microrrótulo → cierre → sonido → voz opcional.
 */
...
import type { CtxAgentico, VentanaAgentico } from "./tipos";
...
const GUION: GuionSuperficie<CtxAgentico> = {
  replay: (win, frame) => aplicarReplay(win as VentanaAgentico, frame),
  postProceso,
  cursor: { tramos: tramosCursor, clics: CLICS },
};

export const DemoAgentico: React.FC = () => {
  const frame = useCurrentFrame();
  return (
    <AbsoluteFill style={{ background: TV.surface2, color: TV.text, fontFamily: FUENTE.sans }}>
      {frame < ACTOS.cierre ? <Grano ancho={LIENZO.w} alto={LIENZO.h} id="grano-agentico" /> : null}
      ...
      {CON_VOZ ? (
        <Sequence layout="none">
          <Audio src={staticFile("voz/demo-agentico.mp3")} />
        </Sequence>
      ) : null}
    </AbsoluteFill>
  );
