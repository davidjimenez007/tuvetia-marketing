/**
 * DemoComunicaciones · la composición (prompts/demo-comunicaciones.md sobre la plantilla): reel 1080×1920 ·
 * 30 fps · 1500 frames, sólo escritorio. Capas (plantilla §3): nieve con grano → ventana de escritorio
 * (plano general con cromo al abrir; la lista, el hilo, el correo e Integraciones después) → banda de texto
 * + microrrótulo → cierre → sonido → voz opcional.
 */
...
import type { CtxComunicaciones, VentanaComunicaciones } from "./tipos";
...
const GUION: GuionSuperficie<CtxComunicaciones> = {
  replay: (win, frame) => aplicarReplay(win as VentanaComunicaciones, frame),
  postProceso,
  cursor: { tramos: tramosCursor, clics: CLICS },
};

export const DemoComunicaciones: React.FC = () => {
  const frame = useCurrentFrame();
  return (
    <AbsoluteFill style={{ background: TV.surface2, color: TV.text, fontFamily: FUENTE.sans }}>
      {frame < ACTOS.cierre ? <Grano ancho={LIENZO.w} alto={LIENZO.h} id="grano-comunicaciones" /> : null}
      ...
      {CON_VOZ ? (
        <Sequence layout="none">
          <Audio src={staticFile("voz/demo-comunicaciones.mp3")} />
        </Sequence>
      ) : null}
    </AbsoluteFill>
  );
